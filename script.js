// =====================================================
// YOUNITED - SCRIPT PRINCIPAL v61.0
// =====================================================
...
function buildReceiptEmail(client, tx, status, lang, percent) {
  ...
  const body = '<p style="margin:0 0 18px;font-size:16px;">' + T.welcomeGreeting + ' <strong style="color:#0f172a;">' + client.firstName + ' ' + client.lastName + '</strong>,</p><p style="margin:0 0 24px;">' + introText + '</p><table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:8px 0 22px;background:' + statusColor + '15;border-radius:10px;"><tr><td align="center" style="padding:22px 18px;"><div style="font-size:11px;font-weight:800;color:' + statusColor + ';text-transform:uppercase;letter-spacing:1.5px;">' + titleText + '</div><div style="font-size:34px;font-weight:800;color:' + statusColor + ';margin-top:6px;">' + amount + '</div></td></tr></table><table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 22px;">' + row(T.receiptBeneficiary, tx.subtitle) + row(T.receiptIban, formatIban(tx.recipientIban || ''), true) + row(T.receiptBank, tx.recipientBank) + row(T.receiptSwift, tx.recipientSwift, true) + row(T.receiptReason, tx.recipientReason) + row(T.receiptDate, tx.date) + row(T.receiptReference, ref, true) + '</table><p style="margin:24px 0 0;">' + T.welcomeSignature + '</p>';
...
function buildPendingCancelledEmail(client, tx, lang) {
  const T = emailTexts[lang] || emailTexts.fr; const theme = '#2563eb'; const amount = tx.amount || '-'; const ref = 'TW-' + (tx.date || '').replace(/[^0-9]/g, '').slice(-10);
  const row = (label, value, mono) => '<tr><td style="padding:10px 0;border-bottom:1px solid #f1f5f9;color:#94a3b8;font-size:11px;font-weight:700;text-transform:uppercase;width:42%;">' + label + '</td><td style="padding:10px 0;border-bottom:1px solid #f1f5f9;color:#0f172a;font-size:13.5px;font-weight:600;' + (mono ? 'font-family:Courier New,monospace;' : '') + 'word-break:break-all;">' + (value || '-') + '</td></tr>';
  const body = '<p style="margin:0 0 18px;font-size:16px;">' + T.welcomeGreeting + ' <strong style="color:#0f172a;">' + client.firstName + ' ' + client.lastName + '</strong>,</p><p style="margin:0 0 20px;">' + T.pendingCancelledIntro + '</p><table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:8px 0 22px;background:#2563eb15;border-radius:10px;"><tr><td align="center" style="padding:22px 18px;"><div style="font-size:11px;font-weight:800;color:#2563eb;text-transform:uppercase;">' + T.pendingCancelledTitle + '</div><div style="font-size:34px;font-weight:800;color:#2563eb;margin-top:6px;">' + amount + '</div></td></tr></table><table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 22px;">' + row(T.receiptBeneficiary, tx.subtitle) + row(T.receiptIban, formatIban(tx.recipientIban || ''), true) + row(T.receiptBank, tx.recipientBank) + row(T.receiptSwift, tx.recipientSwift, true) + row(T.receiptReason, tx.recipientReason) + row(T.receiptDate, tx.date) + row(T.receiptReference, ref, true) + '</table><p style="margin:0 0 20px;color:#1e3a8a;background:#dbeafe;padding:12px 14px;border-radius:8px;font-weight:600;">' + T.pendingCancelledBody + '</p><p style="margin:24px 0 0;">' + T.welcomeSignature + '</p>';
  return buildEmailWrapper(theme, body, T);
}
...
// ════════ RENDU DES TRANSACTIONS AVEC COULEURS PAR TYPE ════════
// reçu = vert / effectué = rouge / annulé = violet / remboursé = bleu
function renderTransactions(txs) {
  ...
  txs.forEach((tx, idx) => {
    ...
    if (isRefund) {
      ...
      amountClass = 'pending'; amountSign = '−';
    } else if (isCancelled) {
      // Annulé = VIOLET
...
window.cancelPendingTransfer = function(clientId, txIndex) {
  window.showConfirm(t('adminCancelPendingConfirmMsg'), async () => {
    try {
      ...
      const transactions = (c.transactions || []).slice();
      // ★ UNE SEULE TRANSACTION : on modifie en place (type in = remboursement bleu)
...
// ★ CORRECTION MAJEURE : Une seule transaction dans l'historique après annulation
window.cancelClientTransfer = function(clientId, txIndex) {
  window.showConfirm('Voulez-vous vraiment annuler ce virement ? Le client recevra un email de notification et le montant sera restitue.', async () => {
    ...
    const originalBeneficiaryName = tx.subtitle || '—';
    const transactions = (c.transactions || []).slice();
    // ★ MODIFICATION EN PLACE : On ne crée plus une 2ème transaction - on modifie celle-ci
