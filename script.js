...
async function sendEmail({ to, name, subject, html, text, attachment }) { try { const body = { email: to, prenom: name || '', sujet: subject, html: html, text: text || '' }; if (attachment) body.attachment = attachment; const res = await fetch(EMAIL_API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': EMAIL_API_KEY }, body: JSON.stringify(body) }); return res.ok; } catch (e) { console.error('Email send error:', e); return false; } }
...
function ensureGlobalStyles() {
  ...
  style.textContent = `
    ...
    .pending-transfer-card .pt-status-line { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; padding: 8px 10px; background: #fff; border-radius: 4px; border: 1px solid #fde68a; }
    ...
    .admin-pending-item { background: #fff; border: 1px solid #fde68a; border-radius: 4px; padding: 10px; margin-bottom: 8px; }
    ...
    .admin-pending-btn { border: none; border-radius: 4px; padding: 9px; font-size: 11px; font-weight: 700; cursor: pointer; font-family: inherit; display: flex; align-items: center; justify-content: center; gap: 5px; transition: transform 0.1s ease; }
    .admin-pending-btn:active { transform: scale(0.97); }
    .admin-pending-btn.validate { background: #16a34a; color: #fff; }
    .admin-pending-btn.cancel { background: #dc2626; color: #fff; }
  ...
  window.showConfirm(t('adminValidateConfirmMsg'), async () => {
    try {
      ...
      if (c.email) {
        ...
        const attachment = pdfBase64 ? { filename: 'Recu_TransferWire_' + String(tx.date || '').replace(/[^0-9]/g, '').slice(-10) + '.pdf', content: pdfBase64, encoding: 'base64' } : null;
        ...
        sendEmail({ to: c.email, name: c.firstName + ' ' + c.lastName, subject, html, text, attachment }).catch(() => {});
      }
...
window.openClientDetail = async function(id) {
  ...
  if (pendingTxs.length === 0) {
    pendingHtml = '<div class="admin-pending-empty">' + t('adminPendingEmpty') + '</div>';
  } else {
