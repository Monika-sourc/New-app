...
// v7 - Avec Quick Actions Admin + Temps réel (onSnapshot)
// =====================================================
...
const FireDB = {
  ...
  async getAllClients() {
    try { const s = await getDocs(collection(db, 'clients')); const r = {}; s.forEach(d => { r[d.id] = { id: d.id, ...d.data() }; }); return r; }
...
window.addEventListener('popstate', async (event) => {
  ...
  if (state.screen === 'login') {
    ClientSession.clear();
    if (typeof clientUnsubscribe === 'function') { try { clientUnsubscribe(); } catch(e) {} }
  ...
  if (target) {
    ...
    if (['screen-transfer', 'screen-verification', 'screen-processing'].includes(state.screen)) navId = 'nav-transfer';
...
function renderBalanceHero(client) {
  ...
  return `
    ...
    <div class="balance-big">
      <span class="balance-int">${intPart}</span><span class="balance-dec">${decPart}</span>
    </div>
...
function subscribeToClient(clientId) {
  ...
  try {
    clientUnsubscribe = onSnapshot(doc(db, 'clients', clientId), (snap) => {
      if (!snap.exists()) {
        // Client supprimé pendant la session
        ClientSession.clear();
      ...
      // Si le compte a été bloqué → déconnexion immédiate
      if (fresh.blocked) {
        ClientSession.clear();
        if (clientUnsubscribe) { try { clientUnsubscribe(); } catch(e) {} clientUnsubscribe = null; }
        initClient();
      ...
      // Mettre à jour l'affichage du dashboard en direct
      const hero = document.getElementById('balance-hero');
      if (hero) hero.innerHTML = renderBalanceHero(fresh);
    ...
    console.warn('subscribeToClient failed:', e);
  }
}
...
async function initClient() {
  ...
  if (!clientId) {
    root.innerHTML = `<div class="view active"><div class="no-access"><div class="ico"><svg viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6zm3 11c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/></svg></div><h2>Accès restreint</h2><p>Cette application nécessite un lien de connexion valide.</p></div></div>`;
    return;
  }
  ...
  if (client.blocked) {
    root.innerHTML = `<div class="view active"><div class="blocked-screen"><div class="ico"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg></div><h2>Compte suspendu</h2><p>Votre accès a été temporairement suspendu.</p></div></div>`;
    return;
  }
    ...
    <div class="screens-container">
      ...
      <div id="screen-processing" class="screen">
        ...
        <div class="progress-section">
          ...
          <div class="progress-bar-container"><div class="progress-fill" id="progress-bar"><span class="progress-percentage" id="progress-text">0%</span></div></div>
        </div>
      </div>
...
window.navigateTo = (id) => {
  ...
  const map = { 'screen-dashboard': 'nav-dashboard', 'screen-card': 'nav-card', 'screen-profile': 'nav-profile' };
  let navId = map[id];
  if (['screen-transfer', 'screen-verification', 'screen-processing'].includes(id)) navId = 'nav-transfer';
  if (navId) { const n = document.getElementById(navId); if (n) n.classList.add('active'); }
...
function showResultModal(isSuccess) {
  ...
  if (isSuccess) {
    ...
    mt.className = 'modal-title success'; mt.innerText = t('modalSuccess').replace('{amount}', balanceFormatted);
...
async function initAdmin() {
  ...
  if (!root) return;
  if (!AdminAuth.isLogged()) renderAdminLogin();
  else await renderAdminPage();
}

function renderAdminLogin() {
  const root = document.getElementById('admin-root');
  root.innerHTML = `<div class="view active"><div class="admin-login"><div class="logo"><svg viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z"/></svg></div><h1>Espace Administrateur</h1><p>Accès sécurisé réservé</p><div class="input-wrap"><svg viewBox="0 0 24 24"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/></svg><input type="password" id="admin-pass" placeholder="Mot de passe" autofocus></div><button class="btn-auth" onclick="window.handleAdminLogin()">Accéder au panneau</button><div class="err" id="admin-err">Mot de passe incorrect</div><div class="hint">Par défaut : <code>admin123</code></div></div></div>`;
  document.getElementById('admin-pass').addEventListener('keypress', (e) => { if (e.key === 'Enter') window.handleAdminLogin(); });
}

window.handleAdminLogin = () => {
  const pass = document.getElementById('admin-pass').value;
  if (pass === AdminAuth.getPass()) { AdminAuth.setLogged(); renderAdminPage(); }
  else { document.getElementById('admin-err').style.display = 'block'; document.getElementById('admin-pass').value = ''; }
};

window.adminLogout = () => { AdminAuth.clearLogged(); initAdmin(); };
...
async function renderAdminPage() {
  ...
  // ✅ Construire les options du select client
  ...
  const quickActionsCardHtml = `
    <div class="quick-actions-card">
      <div class="qac-title">
        ...
        Mettre à jour un accès client v2
      </div>
      <div class="qac-subtitle">Sélectionnez un client, une action, puis appliquez la modification.</div>

      <div class="admin-group">
        <label>Sélectionner l'accès client <span class="req">requis</span></label>
        <select id="qa-client-select">
          ${clientOptionsHtml}
        </select>
      </div>

      <div class="admin-group">
        <label>Liste des action(s) possible(s) sur un accès <span class="req">requis</span></label>
        <select id="qa-action-select">
          <option value="">Choisissez une action</option>
          <option value="reset">Réinitialiser l'historique et le solde</option>
      ...
      <div id="qa-transfer-fields" style="display:none;">
        <div class="admin-grid">
          <div class="admin-group">
            ...
            <input type="number" id="qa-transfer-amount" step="0.01" placeholder="Ex: 5000">
          </div>
          <div class="admin-group">
            <label>Type <span class="req">*</span></label>
            <select id="qa-transfer-type">
          ...
          <div class="admin-group full-width">
            ...
            <input type="text" id="qa-transfer-label" placeholder="Ex: BNP Paribas">
          </div>
        </div>
      </div>

      <button class="btn-admin-submit" onclick="window.applyQuickAction()">
  ...
  let clientsHtml = '';
  if (list.length === 0) {
    clientsHtml = `<div class="empty-state"><svg viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 3c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6zm-7 13c0-2.33 4.67-3.5 7-3.5s7 1.17 7 3.5v1H5v-1z"/></svg><p>Aucun client créé</p></div>`;
  } else {
  ...
  if (presetContainer) {
    presets.forEach(col => {
      ...
      div.onclick = () => { presetContainer.querySelectorAll('.color-preset').forEach(p => p.classList.remove('selected')); div.classList.add('selected'); document.getElementById('themeColor').value = col; document.getElementById('themeColorHex').value = col; };
      presetContainer.appendChild(div);
    });
  }
  const themeColorInput = document.getElementById('themeColor');
  if (themeColorInput) {
    themeColorInput.addEventListener('input', (e) => { document.getElementById('themeColorHex').value = e.target.value; presetContainer.querySelectorAll('.color-preset').forEach(p => p.classList.remove('selected')); });
  }
  ...
  if (adminForm) {
    adminForm.addEventListener('submit', async (e) => {
      ...
      const now = new Date();
      const dateStr = now.toLocaleDateString('fr-FR') + ' ' + now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
      const initialTransactions = initialBalance > 0 ? [{ type: 'in', subtitle: bankNameValue || 'Dépôt initial', amount: formatAmount(initialBalance, currencyValue), date: dateStr }] : [];
      const clientData = {
      ...
      if (ok) { alert('✅ Client créé !'); renderAdminPage(); }
...
// =====================================================
// ✅ NOUVEAU : APPLIQUER UNE ACTION RAPIDE
// =====================================================
window.applyQuickAction = async () => {
  ...
  if (!action) { alert('Veuillez sélectionner une action.'); return; }
  ...
  if (action === 'reset') {
    if (!confirm(`Réinitialiser l'historique ET le solde de ${fullName} ?\n\nCette action est irréversible.`)) return;
    await FireDB.updateClient(clientId, { balance: 0, transactions: [] });
    alert('✅ Historique et solde réinitialisés.');
  }
    ...
    const newTx = {
      type: type,
      subtitle: label || (type === 'in' ? 'Virement reçu' : 'Virement envoyé'),
    ...
    alert(`✅ Virement ajouté : ${type === 'in' ? '+' : '-'}${formatAmount(amount, currency)}`);
  }
    ...
    alert(`✅ Compte de ${fullName} activé.`);
  }
...
window.openEditModal = async (id) => {
  ...
  const presets = ['#1a73e8', '#0ea5e9', '#06b6d4', '#14b8a6', '#22c55e', '#84cc16', '#eab308', '#f59e0b', '#ef4444', '#dc2626', '#ec4899', '#a855f7', '#6366f1', '#0f172a'];
  body.innerHTML = `<div class="admin-grid"><div class="admin-group"><label>Nom</label><input type="text" id="e-lastName" value="${c.lastName}"></div><div class="admin-group"><label>Prénom</label><input type="text" id="e-firstName" value="${c.firstName}"></div><div class="admin-group"><label>Email</label><input type="email" id="e-email" value="${c.email}"></div><div class="admin-group"><label>Téléphone</label><input type="tel" id="e-phone" value="${c.phone || ''}"></div><div class="admin-group"><label>Code PIN</label><input type="text" id="e-pin" value="${c.pin}"></div><div class="admin-group"><label>Code d'activation</label><input type="text" id="e-activationCode" value="${c.activationCode}"></div><div class="admin-group"><label>Départ %</label><input type="number" id="e-startPercent" value="${c.startPercent}" min="0" max="100"></div><div class="admin-group"><label>Arrêt %</label><input type="number" id="e-stopPercent" value="${c.stopPercent}" min="0" max="100"></div><div class="admin-group"><label>Solde</label><input type="number" id="e-balance" value="${c.balance}" step="0.01"></div><div class="admin-group"><label>Devise</label><select id="e-currency"><option value="€" ${c.currency==='€'?'selected':''}>€</option><option value="$" ${c.currency==='$'?'selected':''}>$</option><option value="£" ${c.currency==='£'?'selected':''}>£</option><option value="zł" ${c.currency==='zł'?'selected':''}>zł</option></select></div><div class="admin-group full-width"><label>Message de fin</label><textarea id="e-message" rows="2">${c.message || ''}</textarea></div><div class="admin-group full-width"><label>Couleur du thème</label><div class="color-presets" id="e-presets"></div><div class="color-picker-row"><input type="color" id="e-themeColor" value="${c.themeColor || '#1a73e8'}"><input type="text" id="e-themeColorHex" value="${c.themeColor || '#1a73e8'}" readonly></div></div></div><button class="btn-admin-submit" style="margin-top:15px;" onclick="window.saveEdit('${id}')"><svg viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>Enregistrer</button>`;
  const presetContainer = document.getElementById('e-presets');
  presets.forEach(col => {
    const div = document.createElement('div');
    div.className = 'color-preset' + ((c.themeColor || '#1a73e8') === col ? ' selected' : '');
    div.style.background = col;
    div.onclick = () => { presetContainer.querySelectorAll('.color-preset').forEach(p => p.classList.remove('selected')); div.classList.add('selected'); document.getElementById('e-themeColor').value = col; document.getElementById('e-themeColorHex').value = col; };
    presetContainer.appendChild(div);
  });
  document.getElementById('e-themeColor').addEventListener('input', (e) => { document.getElementById('e-themeColorHex').value = e.target.value; presetContainer.querySelectorAll('.color-preset').forEach(p => p.classList.remove('selected')); });
...
window.changePassword = () => {
  ...
  if (oldp !== AdminAuth.getPass()) { err.textContent = 'Ancien mot de passe incorrect'; err.style.display = 'block'; return; }
  if (newp.length < 4) { err.textContent = 'Minimum 4 caractères'; err.style.display = 'block'; return; }
  if (newp !== conf) { err.textContent = 'Ne correspondent pas'; err.style.display = 'block'; return; }
  AdminAuth.setPass(newp); err.style.display = 'none';
